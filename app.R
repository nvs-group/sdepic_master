...
#Master filtered table  
  table_var <- reactive({
    mast_temp <- master1
    if(!is.null(input$epic.school.name )) {
      mast_temp <- filter(mast_temp, school.name %in% input$epic.school.name)
    }
    if(!is.null(input$epic.degree.name )) {
      mast_temp <- filter(mast_temp, degree.name %in% input$epic.degree.name)
    }
    if(!is.null(input$epic.state)) {
      mast_temp <- filter(mast_temp, State %in% input$epic.state)
    }
    if(!is.null(input$epic.experience)) {
      mast_temp <- filter(mast_temp, Experience %in% input$epic.experience)
    }
    if(!is.null(input$epic.occ.name)) {
      mast_temp <- filter(mast_temp, occ.name %in% input$epic.occ.name)
    }
    if(!is.null(input$epic.entry.degree)) {
      mast_temp <- filter(mast_temp, entry.degree %in% input$epic.entry.degree)
    }
    if(!is.null(input$epic.cip.name)) {
      mast_temp <- filter(mast_temp, cip.name %in% input$epic.cip.name)
    }
    if(!is.null(input$epic.cip.cat)){
      cip_var <- cip1$CIP_Code[cip1$CIP_Category %in% input$epic.cip.cat]
      mast_temp <- filter(mast_temp, cip.cat %in% cip_var)
    }
    if(!is.null(input$epic.occ.cat)){
      occ_var <- soc1$SOC_Code[soc1$SOC_Cat_Name %in% input$epic.occ.cat]
      mast_temp <- filter(mast_temp, soc.cat %in% occ_var)
    }
    mast_temp <- filter(mast_temp, InStOff <= input$epic.tuition)
    mast_temp <- filter(mast_temp, X17p >= input$epic.income)
  })
